...
def audio_to_audio_frame_stack(sound_data, frame_length, hop_length_frame):
    ...
    if (len(sound_data_list)) > 1:
      sound_data_array = np.vstack(sound_data_list)
    else:
      sound_data_array = np.array([])
...
def audio_files_to_numpy(audio_dir, list_audio_files, sample_rate, frame_length, hop_length_frame, min_duration):
    ...
    if (total_duration >= min_duration):
        ...
        out = np.vstack(list_sound_array)
    else:
        print(
...
def prediction(weights_path, name_model, audio_dir_prediction, dir_save_prediction, audio_input_prediction_list, sample_rate, min_duration, frame_length, hop_length_frame, n_fft, hop_length_fft):
    ...
    for audio_input_prediction in audio_input_prediction_list:
      # Extracting noise and voice from folder and convert to numpy
      audio_input_prediction = audio_input_prediction.split('/')[-1]
